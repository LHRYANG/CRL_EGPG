...
def creat_vocab(valid = True,min_count = 3,data_root="data"):
    ...
    with open(os.path.join(data_root,"word2idx2.pkl"),'wb') as f:
        pickle.dump(word2idx,f)
    with open(os.path.join(data_root,"idx2word2.pkl"),'wb') as f:
        pickle.dump(idx2word,f)

def token_to_idx(sub_file='/train'):
    word2idx = loadpkl("data2/word2idx.pkl")
    ...
    with open("data2"+sub_file+"/src.pkl",'wb') as f:
        pickle.dump(src_idx,f)

    with open("data2"+sub_file+"/trg.pkl",'wb') as f:
        pickle.dump(trg_idx,f)
...
def id_to_bertid(sub_file="/test"):
    ...
    idx2word = loadpkl("data2/idx2word.pkl")
    input_path = "data2"+sub_file+"/src.pkl"
    output_path = "data2"+sub_file+"/trg.pkl"
...
def find_exm(sub_file="/test"):
    ...
    tags = loadpkl("data2"+sub_file+"/trg_pos.pkl")
    trgs = loadpkl("data2"+sub_file+"/trg.pkl")
    ...
    for i in range(len(trgs)):
        ...
        if i % 1000 == 0:
            print(i)
        for j in range(max(0,i-20000),min(i+20000,len(trgs))):
            if i != j:
                len_j = len(trgs[j])
                if abs(len_i - len_j) > 1:
                    continue
