...
def get_nll_loss(fc_out, fc_label, weight=None, label=None, reduction='mean', ope='log_softmax'):
    '''
    ...
    if len(fc_label.size()) == 1:
        ...
        loss = (label * F.nll_loss(input=fc_out.transpose(1, 2), target=fc_label,
                                   reduction='none')).sum(dim=-1) / (label.sum(dim=-1) + min_num)
        if reduction == 'mean':
            return loss.mean()
        else:
            return loss
